 ...
 *
 * 来源：力扣（LeetCode）
 * 链接：<a href="https://leetcode-cn.com/problems/longest-absolute-file-path">https://leetcode-cn.com/problems/longest-absolute-file-path</a>
 * 著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
 * @Date Create in 2022/04/20 14:09
 */
    ...
    public static int method(String input) {
        ...
        for (String str : arr) {
            ...
            while (str.startsWith("\t")){
                ...
                path += pathList.get(deep);
                deep ++;
            }
            if (!str.contains(".")){
                if (pathList.size() <= deep){
                    pathList.add(str.length());
                }else {
                    pathList.set(deep, str.length());
                }
            }else {
                if (deep >= 1){
                    path++;
